            ...
            if (vTv > 1e-28)
            {
                ...
                for (int i = 0; i < m - k; ++i)
                {
                    ...
                    for (; j + 1 < n - k; j += 2)
                    {
                        ...
                        float64x2_t b_vec = vld1q_f64(bptr);
                        float64x2_t w_vec = vld1q_f64(&w[j]);
                        b_vec = vsubq_f64(b_vec, vmulq_f64(scale_vec, w_vec));
                        vst1q_f64(bptr, b_vec);
                    }

                    for (; j < n - k; ++j)
                    {
                        B.at(k + i, k + j) -= scale * w[j];
                    }
                }
                ...
                for (int i = 0; i < m; ++i)
                {
                    ...
                    for (; j + 1 < m - k; j += 2)
                    {
                        ...
                        float64x2_t u_vec = vld1q_f64(uptr);
                        float64x2_t v_vec = vld1q_f64(&v[j]);
                        u_vec = vsubq_f64(u_vec, vmulq_f64(scale_vec, v_vec));
                        vst1q_f64(uptr, u_vec);
                    }

                    for (; j < m - k; ++j)
                    {
                        U.at(i, k + j) -= scale * v[j];
                    }
                }
                ...
                if (vTv > 1e-28)
                {
                    ...
                    for (int i = 0; i < m - k; ++i)
                    {
                        ...
                        for (; j + 1 < n - k - 1; j += 2)
                        {
                            ...
                            float64x2_t b_vec = vld1q_f64(bptr);
                            float64x2_t v_vec = vld1q_f64(&v[j]);
                            sum_vec = vaddq_f64(sum_vec, vmulq_f64(b_vec, v_vec));
                        }

                        double tmp[2];
                        vst1q_f64(tmp, sum_vec);
                    ...
                    for (int i = 0; i < m - k; ++i)
                    {
                        ...
                        for (; j + 1 < n - k - 1; j += 2)
                        {
                            ...
                            float64x2_t b_vec = vld1q_f64(bptr);
                            float64x2_t v_vec = vld1q_f64(&v[j]);
                            b_vec = vsubq_f64(b_vec, vmulq_f64(scale_vec, v_vec));
                            vst1q_f64(bptr, b_vec);
                        }

                        for (; j < n - k - 1; ++j)
                        {
                            B.at(k + i, k + 1 + j) -= scale * v[j];
                        }
                    }
                    ...
                    for (int i = 0; i < n; ++i)
                    {
                        ...
                        for (; j + 1 < n - k - 1; j += 2)
                        {
                            ...
                            float64x2_t row_vec = vld1q_f64(Vptr);
                            float64x2_t v_vec = vld1q_f64(&v[j]);
                            sum_vec = vaddq_f64(sum_vec, vmulq_f64(row_vec, v_vec));
                        }
                    ...
                    for (int i = 0; i < n; ++i)
                    {
                        ...
                        for (; j + 1 < n - k - 1; j += 2)
                        {
                            ...
                            vst1q_f64(Vptr, row_vec);
                        }

                        for (; j < n - k - 1; ++j)
                        {
                            V.at(i, k + 1 + j) -= scale * v[j];
                        }
                    }
                }
            }
