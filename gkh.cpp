    ...
    static void apply_left_rows(Matrix &M, int r0, int r1, double c, double s)
{
    ...
    const float64x2_t c_vec = vdupq_n_f64(c);
    const float64x2_t s_vec = vdupq_n_f64(s);
    const float64x2_t neg_s_vec = vdupq_n_f64(-s);
    ...
    for (; j + 1 < cols; j += 2)
    {
        ...
        float64x2_t a_vec = vld1q_f64(row0);
        float64x2_t b_vec = vld1q_f64(row1);

        float64x2_t new0 = vaddq_f64(vmulq_f64(c_vec, a_vec),
                                      vmulq_f64(s_vec, b_vec));

        float64x2_t new1 = vaddq_f64(vmulq_f64(neg_s_vec, a_vec),
                                      vmulq_f64(c_vec, b_vec));
...
static void apply_left_rows_banded(Matrix &M, int r0, int r1, double c, double s)
{
    const int cols = M.cols();

    int j_begin = std::max(0, r0 - 2);
    int j_end = std::min(cols - 1, r1 + 3);
    ...
    static void apply_right_cols_banded(Matrix &M, int c0, int c1, double c, double s)
{
    ...
    int i_begin = std::max(0, c0 - 2);
    int i_end = std::min(rows - 1, c1 + 2);
    ...
    static void one_block_step(Matrix &U, Matrix &B, Matrix &V, int l, int r)
    {
        ...
        apply_left_rows_banded(B, l, l + 1, c, s);
        ...
        for (int k = l + 1; k <= r - 1; ++k)
        {
            ...
            apply_left_rows_banded(B, k, k + 1, c, s);
    ...
    static bool chase_zero_diagonal(Matrix &U, Matrix &B, Matrix &V, int k, double tol)
    {
        ...
        for (int i = k; i <= n - 2; ++i)
        {
            ...
            if (i + 1 < m)
            {
               ...
               apply_left_rows_banded(B, i, i + 1, c, s);
